...
async function postJsonWithTimeout(url, token, body) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`pair ${response.status}: ${text.slice(0, 1000)}`);
    }
...
function sanitizedResult(result, requestedUsername, elapsedMs) {
  return {
    ok: true,
    elapsedMs,
    requestedUsername,
...
try {
  ...
  const result = await postJsonWithTimeout(`${baseUrl.replace(/\/$/, "")}/api/pair`, token, {
    handshake,
    username,
    network,
  });
  console.log(JSON.stringify(sanitizedResult(result, username, Date.now() - started), null, 2));
} catch (error) {

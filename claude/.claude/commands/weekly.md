...
Add an entry to the weekly work log in the team repo at `~/team/worklog/`.

Steps:
1. Determine today's date and the Monday of the current week using: `date -d "$(date +%Y-%m-%d) -$(( $(date +%u) - 1 )) days" +%Y-%m-%d`
2. The weekly file is at `~/team/worklog/<YYYY-MM-DD>.md` where the date is that Monday.
...
Notes:
- Always write to `~/team/worklog/`, whatever the current working directory is. Never write the weekly log anywhere else, and never under `~/github/notes/`.
- This is a personal work log. It is separate from the generated team briefings under `~/team/briefings/weekly/`.
